...
const SHOWCASE_PRODUCTS = [
  { 
    ...
    // Cinnabons usa 'options' para sabor+precio
    options: [
  ...
  { 
    ...
    // Cupcakes usa 'options' solo para texto (sabor)
    options: [
  ...
  { id: 3, name: "Alfajores Cubanos", price: 20, description: "Media docena de alfajores rellenos con dulce de leche.", images: ["/img/alfajores.jpeg"] },
  { 
    id: 4, 
    name: "Torta Decorada", 
    ...
    // Torta usa 'sizes' (precio) y 'flavors' (texto)
    sizes: [
          ...
          {cart.length > 0 && (
            <div className="fixed bottom-4 left-4 right-4 sm:left-auto sm:right-6 sm:bottom-6 sm:w-80 bg-white/95 backdrop-blur-md shadow-2xl rounded-2xl p-4 sm:p-5 z-50 border-2 border-black">
              ...
              <ul className="text-sm space-y-2 max-h-40 overflow-y-auto mb-4 pr-1">
                {cart.map((i, idx) => (
                  <li key={idx} className="flex justify-between border-b border-black/20 pb-2 last:border-0 text-black">
                    <div><span className="font-medium">{i.name} x{i.qty}</span>{i.extras && <p className="text-xs text-black/60 italic">({i.extras})</p>}</div>
              ...
              {selected.options && (
                <>
                  ...
                  <select value={selectedOption} onChange={e => setSelectedOption(e.target.value)} className="w-full border-2 border-black rounded px-3 py-2 mb-3 bg-white/50" required>
                    ...
                    {selected.options.map(o => <option key={o.value} value={o.value}>{o.label} {o.price ? `- $${o.price}` : ''}</option>)}
                  </select>
                </>
              )}
